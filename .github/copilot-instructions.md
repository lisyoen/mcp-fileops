# GitHub Copilot Instructions

## 다른 프로젝트에서 이 지시사항 사용하기
이 지시사항 시스템을 다른 프로젝트에서도 사용하려면:
1. 이 프로젝트의 `.github/copilot-instructions.md` 파일을 새 프로젝트의 `.github/` 폴더로 복사
2. `.github/.prompt.txt` 파일도 함께 복사
3. 세션 관리 시스템을 사용하려면:
   - `.github/session-manager.md` 복사
   - `.github/current-session.md` 복사
   - `.github/project-context.md` 복사
   - `.github/work-history.md` 복사
   - `.github/sessions/` 폴더 구조 생성
4. 프로젝트별 특성에 맞게 내용 수정

---

git message 는 한국어로 생성해야 합니다.
생각 내용도 한국어로 생성해야 합니다.

지시한 사항을 묻지 않고 ./.github/.prompt.txt 에 업데이트 합니다.
매 작업을 시작할 때마다 ./.github/.prompt.txt 를 읽고 내용을 따릅니다.

모르는 내용이 있으면 mcp-websearch의 도구를 사용하여 직접 웹 검색을 수행합니다.
- `mcp_mcp-websearch_web_search` - 프로그래밍 관련 질문, API 사용법, 라이브러리 문서 등을 직접 검색
- `mcp_mcp-websearch_web_fetch` - 특정 URL의 문서나 가이드를 가져와서 읽기
- 사용자에게 ChatGPT에게 물어보라고 요청하지 않음

## MCP Tools 사용 규칙

### 사용 가능한 MCP 서버
VSCode의 `mcp.json`에 다음 MCP 서버들이 등록되어 있습니다:

1. **mcp-websearch** - 웹 검색 및 크롤링
   - `mcp_mcp-websearch_web_search` - Bing 검색
   - `mcp_mcp-websearch_web_fetch` - URL 페이지 가져오기
   - `mcp_mcp-websearch_web_scrape` - CSS 선택자로 스크래핑
   - `mcp_mcp-websearch_web_crawl` - BFS 크롤링

2. **mcp-fileops** - 로컬 파일 작업
   - `mcp_mcp-fileops_read_file` - 파일 읽기
   - `mcp_mcp-fileops_write_to_file` - 파일 쓰기
   - `mcp_mcp-fileops_append_to_file` - 파일에 추가
   - `mcp_mcp-fileops_list_directory` - 디렉토리 목록
   - `mcp_mcp-fileops_delete_file` - 파일 삭제
   - `mcp_mcp-fileops_open_file_vscode` - VSCode에서 파일 열기

### 파일 작업 시 MCP Tool 사용
작업 영역 외부의 파일에 접근할 때는 반드시 mcp-fileops의 MCP tool을 사용합니다:

1. **파일 읽기**: `mcp_mcp-fileops_read_file`
   - 작업 영역 밖의 파일 내용을 읽을 때 사용
   - 예: 다른 프로젝트의 파일, 시스템 파일 등
   ```
   Tool: mcp_mcp-fileops_read_file
   Args: { "path": "D:/other-project/file.txt" }
   ```

2. **파일 쓰기**: `mcp_mcp-fileops_write_to_file`
   - 작업 영역 밖의 파일에 내용을 쓸 때 사용 (덮어쓰기)
   - **중요**: 파일 작성 후 반드시 PowerShell로 VSCode에서 열기
   ```
   Tool: mcp_mcp-fileops_write_to_file
   Args: { "path": "D:/output/result.txt", "content": "..." }
   
   # 파일 작성 후 즉시 VSCode에서 열기
   Tool: run_in_terminal
   Command: & "$env:LOCALAPPDATA\Programs\Microsoft VS Code\Code.exe" "D:/output/result.txt"
   ```

3. **파일에 추가**: `mcp_mcp-fileops_append_to_file`
   - 작업 영역 밖의 파일 끝에 내용을 추가할 때 사용
   - **중요**: 파일 수정 후 반드시 PowerShell로 VSCode에서 열기
   ```
   Tool: mcp_mcp-fileops_append_to_file
   Args: { "path": "D:/logs/app.log", "content": "..." }
   
   # 파일 수정 후 즉시 VSCode에서 열기
   Tool: run_in_terminal
   Command: & "$env:LOCALAPPDATA\Programs\Microsoft VS Code\Code.exe" "D:/logs/app.log"
   ```

4. **디렉토리 목록**: `mcp_mcp-fileops_list_directory`
   - 작업 영역 밖의 디렉토리 내용을 조회할 때 사용
   ```
   Tool: mcp_mcp-fileops_list_directory
   Args: { "path": "D:/projects" }
   ```

5. **파일 삭제**: `mcp_mcp-fileops_delete_file`
   - 작업 영역 밖의 파일을 삭제할 때 사용
   ```
   Tool: mcp_mcp-fileops_delete_file
   Args: { "path": "D:/temp/old-file.txt" }
   ```

**필수 규칙**: `write_to_file` 또는 `append_to_file` 사용 후 반드시 PowerShell 명령으로 파일을 VSCode에서 열어야 합니다!

### 파일 편집 작업 (중요!)
파일을 편집해야 할 때는 **직접 수정하지 않고** VSCode에서 열어야 합니다:

**방법**: PowerShell 명령으로 Code.exe를 실행하여 파일을 VSCode에서 엽니다
```powershell
& "$env:LOCALAPPDATA\Programs\Microsoft VS Code\Code.exe" "D:/path/to/file.txt"
```

또는 MCP tool 사용:
```
Tool: mcp_mcp-fileops_open_file_vscode
Args: { "path": "D:/path/to/file.txt" }
```

**편집 프로세스**:
1. 파일을 VSCode에서 열기
2. 사용자가 수동으로 편집하도록 안내
3. 파일을 읽어서 변경사항 확인 (필요시)

### 작업 영역 내부 파일
작업 영역(workspace) 내부의 파일은 기존 도구 사용:
- `read_file` - 작업 영역 내 파일 읽기
- `create_file` - 새 파일 생성
- `replace_string_in_file` - 파일 내용 수정
- `list_dir` - 디렉토리 목록

### 사용 우선순위
1. **작업 영역 내부**: 기본 VSCode 도구 사용
2. **작업 영역 외부 읽기**: `mcp_mcp-fileops_read_file`
3. **작업 영역 외부 쓰기**: `mcp_mcp-fileops_write_to_file` 또는 `append_to_file`
4. **파일 편집 필요**: VSCode에서 열기 (PowerShell 또는 `open_file_vscode`)

## 작업 세션 연속성 관리

### 새 세션/작업 시작 시 필수 절차
1. **Git 동기화**: `git pull`을 실행하여 최신 세션 정보 가져오기
2. **세션 선택**: `.github/sessions/` 디렉토리의 모든 세션 파일을 최신순으로 정렬
   - 각 세션의 일시(파일명의 날짜/시간)와 제목 표시
   - 사용자에게 이어서 작업할 세션 선택 요청
   - 새 세션을 시작할지, 기존 세션을 이어갈지 확인
3. **세션 컨텍스트 로드**: 선택된 세션 파일 및 관련 문서 읽기
   - 선택한 세션의 상세 정보 (`session-ID.md`)
   - 현재 작업 상태 (`.github/current-session.md`)
   - 프로젝트 전체 맥락 (`.github/project-context.md`)
   - 전체 작업 목록 (`.github/todo-list.md`)
4. **작업 목록 표시**: `manage_todo_list` 도구와 `.github/todo-list.md` 파일을 함께 확인
   - `.github/todo-list.md`의 전체 작업 목록 표시
   - 진행 중인 작업, 완료된 작업, 대기 중인 작업을 한눈에 파악
   - 사용자가 우선순위를 결정할 수 있도록 지원

### 작업 시작 시 즉시 수행할 작업 (중요!)
**작업이 중단되어도 이어갈 수 있도록 작업 시작 시 먼저 기록합니다:**

1. **새 세션 파일 생성**: `.github/sessions/session-ID.md`
   - 세션 ID, 날짜, 상태 (진행 중)
   - **작업 목적**: 무엇을 왜 하려고 하는지 명확히 기록
   - **작업 계획**: 어떻게 진행할지 단계별로 기록
   - 현재 문제점 및 개선 방향

2. **current-session.md 업데이트**
   - 현재 세션 ID 변경
   - 진행 중인 작업 내용 기록
   - 작업 목적 요약

3. **session-manager.md 업데이트**
   - 현재 활성 세션 ID 변경
   - 세션 목록에 새 세션 추가 (상태: 진행 중)

4. **TODO 리스트 생성** (manage_todo_list 도구 사용)
   - 작업 단계를 세분화하여 TODO로 관리
   - 각 단계별 진행 상황 추적

**이제 작업을 시작합니다. 작업 중 언제든지 세션 파일을 업데이트하여 진행 상황을 기록합니다.**

### 세션 인식 및 재개 절차
1. **세션 확인**: `.github/session-manager.md`에서 현재 활성 세션 ID 확인
2. **세션 상태 로드**: `.github/current-session.md` 및 해당 세션 파일 읽기
3. **컨텍스트 이해**: `.github/project-context.md`로 프로젝트 전체 맥락 파악
4. **중단된 지점 파악**: 세션 파일에서 마지막 수행 단계 확인
5. **작업 재개**: 다음 단계부터 계속 진행

### 세션별 추적 파일들

1. **세션 관리자**: `.github/session-manager.md`
   - 현재 활성 세션 ID
   - 전체 세션 목록 및 상태
   - 새 세션 생성 규칙

2. **현재 작업 상태**: `.github/current-session.md`
   - 현재 세션 ID 및 요약
   - 진행 중인 작업 내용
   - 마지막 수행한 단계
   - 다음에 해야 할 작업 목록

3. **세션 상세 정보**: `.github/sessions/session-ID.md`
   - 해당 세션의 모든 상세 정보
   - 수행된 작업 및 결정사항
   - 생성/수정된 파일 목록

4. **작업 히스토리**: `.github/work-history.md`
   - 완료된 세션들의 아카이브
   - 주요 변경사항 및 결정 사항
   - 학습한 내용 및 해결된 문제들

5. **프로젝트 컨텍스트**: `.github/project-context.md`
   - 프로젝트 구조 및 아키텍처 이해
   - 중요한 파일들과 그 역할
   - 코딩 스타일 및 컨벤션

6. **작업 목록 (TODO List)**: `.github/todo-list.md`
   - 프로젝트 전체 작업 목록 관리
   - 대기 중, 진행 중, 완료된 작업 분류
   - 각 작업의 상세 설명 및 우선순위
   - 사용자가 지정한 작업들을 세션과 독립적으로 관리

### 작업 완료 시 업데이트 절차

**작업이 완료되면 결과를 기록하고 Git에 동기화합니다:**

1. **세션 파일 완료 처리**: `.github/sessions/session-ID.md`
   - 상태를 "완료"로 변경
   - **작업 결과** 섹션 작성
     - 수행한 작업 내용
     - 생성/수정된 파일 목록
     - 주요 결정사항
     - 학습한 내용
   - 진행 상황 체크리스트 모두 완료 표시

2. **current-session.md 업데이트**
   - 작업 완료 상태 반영
   - "다음에 해야 할 작업" 모두 완료 표시
   - 동기화 상태: "동기화 대기" → "동기화 완료"

3. **session-manager.md 업데이트**
   - 해당 세션 상태를 "완료"로 변경

4. **work-history.md에 아카이브** (선택적)
   - 중요한 세션의 경우 work-history.md에 요약 추가

5. **Git 동기화** (반드시 이 순서로!)
   ```
   git add .github/
   git commit -m "세션 session-ID 완료: [작업 요약]"
   git push
   ```

**중요**: Git 동기화 전에 모든 세션 파일을 먼저 업데이트해야 합니다!

### 작업 중단 시 (예상치 못한 종료)
작업 시작 시 세션 파일을 생성했으므로, 다음 세션에서:
1. `.github/current-session.md`에서 진행 중인 세션 확인
2. 해당 세션 파일 읽고 중단된 지점 파악
3. `.github/todo-list.md`에서 전체 작업 목록 확인
4. TODO 리스트 확인하여 다음 단계부터 재개

### TODO 작업 목록 관리
사용자가 새로운 작업을 지정하면:
1. **즉시 `.github/todo-list.md`에 기록**
   - 작업 제목, 상세 설명, 우선순위 포함
   - 상태: 대기 중 / 진행 중 / 완료 로 분류
2. **세션과 독립적으로 관리**
   - TODO는 여러 세션에 걸쳐 진행될 수 있음
   - 각 세션에서는 TODO 중 일부를 처리
3. **작업 완료 시 상태 업데이트**
   - 완료된 작업을 "완료" 섹션으로 이동
   - 완료 날짜 및 관련 세션 ID 기록

### Git 동기화 절차 (중요!)
**동기화 명령 시 순서**:
1. **먼저** 모든 세션 파일 상태를 "동기화 완료"로 미리 업데이트
2. **그 다음** git add, commit, push 실행
3. 동기화 후 추가 세션 파일 업데이트 금지 (순환 문제 방지)

매 작업 완료 시마다 이 파일들을 업데이트하여 다른 환경에서도 작업을 seamless하게 이어갈 수 있도록 합니다.
